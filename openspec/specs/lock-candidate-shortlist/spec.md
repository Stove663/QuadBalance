...
Build, rank, and present the post-sweep three-slot return-seeking lock shortlist so users can pick a soft-pass configuration and lock it with human sign-off when natural `lockable` is empty.
